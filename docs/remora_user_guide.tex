...
REMORA is a modular tool to monitor runtime resource utilization in HPC environments. As of version 1.5.0 the suite contains code designed to test:
...
Version 1.5.0 is a new release that adds DVS file system support for Cray systems, the option to use a temporary directory that is situated in local storage using the \verb+REMORA_TMPDIR+ variable, an optional file system blacklist, and an improved code structure to facilitate updates. The output directory has been cleaned up so that data is stored in subdirectories by type ( IO / MEM / NETWORK / IO / INFO ), since things were getting confusing for large runs.
...
\node [level 2, below of = c211, xshift=-15pt] (c22) {remora\_report\_mic.sh};

\node [level 3, below of = c3, xshift=15pt] (c31) {extra};
\node [level 3, below of = c31] (c32) {report};
\node [level 5, below of = c32, xshift=15pt] (c321) {show\_time};
\node [level 5, below of = c321] (c322) {show\_final\_report};
