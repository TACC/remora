...
int main(int argc, char *argv[])
{
  ...
  unsigned long long * time_nice_old;
  ...
  if (file != NULL)  {
    ...
    time_nice_old = malloc(sizeof(unsigned long long int) * n_cpus);
    ...
    while (getline(&line_buf, &line_buf_size, file) >= 0) {
      sscanf(line_buf, "%llu %llu %llu %llu", time_usr_old+idx, time_sys_old+idx, time_nice_old+idx, time_tot_old+idx);
      idx++;
    }
  }
  ...
  unsigned long long * time_nice_new = malloc(sizeof(unsigned long long int) * n_cpus);
  ...
  for (int i = 0; i < n_cpus; i++)
    fprintf(file, "%llu %llu %llu %llu\n", time_usr_new[i], time_sys_new[i], time_nice_new[i], time_tot_new[i]);
  ...
  double avg_frac_nice = 0;

  if (is_tmp_file_exists) {

    for (int i = 0; i < n_cpus; i++) {

      frac = ((time_tot_new[i] - time_tot_old[i])-(time_nice_new[i] - time_nice_old[i])) * 100.0 / (time_tot_new[i] - time_tot_old[i]);
      avg_frac_usr += frac;
      ...
      frac = (time_nice_new[i] - time_nice_old[i]) * 100.0 / (time_tot_new[i] - time_tot_old[i]);
      avg_frac_nice += frac;
    }
    avg_frac_usr /= n_cpus;
    avg_frac_sys /= n_cpus;
    avg_frac_nice /= n_cpus;
    ...
    fprintf(file, "%stime %.3f ", "%", 0.5 * (time_new + time_old));
    ...
    fprintf(file, "%snice %.3f ", "%", avg_frac_nice);
